...
type pathLock struct {
	refCount int64      // Number of threads/goroutines owning or waiting on this lock.  Protected by global pathLocksMutex.
...
// unlockPath obtains the pathLock for path.
// The caller must call unlockPath eventually.
...
func (bdc *boltDBCache) view(fn func(tx *bolt.Tx) error) (retErr error) {
	// FIXME??? bolt.Open(bdc.path, 0600, &bolt.Options{ReadOnly: true}) will, if the file does not exist,
	...
	// FIXME?? Hence this preliminary check, which is RACY.
