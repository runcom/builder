...
func TestBlobInfoCacheDir(t *testing.T) {
	...
	for _, c := range []struct {
		...
		{nil, 1, filepath.Join(xdgDataHome, "containers")},
		// There is a context, but it does not override the path.
		{&types.SystemContext{}, 0, systemBlobInfoCacheDir},
		{&types.SystemContext{}, 1, filepath.Join(xdgDataHome, "containers")},
		// Path overridden
		...
		{&types.SystemContext{RootForImplicitAbsolutePaths: rootPrefix}, 1, filepath.Join(xdgDataHome, "containers")},
	...
	for _, c := range []struct {
		...
		{"", homeDir, filepath.Join(homeDir, ".local", "share", "containers")}, // HOME only
		{xdgDataHome, "", filepath.Join(xdgDataHome, "containers")},            // XDG_DATA_HOME only
		{xdgDataHome, homeDir, filepath.Join(xdgDataHome, "containers")},       // both
		{"", "", ""}, // neither
	} {
		if c.xdgDH != "" {
